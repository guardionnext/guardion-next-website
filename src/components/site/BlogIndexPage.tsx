...
import { getBlogIndexContent } from "@/lib/content/blog-index";

export function BlogIndexPage({ locale = "en" }: { locale?: Locale }) {
  const c = getBlogIndexContent(locale);
  const posts = getAllPosts();
  const dateLocale = locale === "zh-Hans" ? "zh-CN" : "en-AU";

  // Blog schema — articles live at the English /blog/<slug> URLs.
  const jsonLd = {
      ...
      <main id="main">
        <section className="grain vignette relative overflow-hidden">
          ...
          <div className="relative z-10 mx-auto max-w-[1280px] px-6 pb-20 pt-40 md:pb-28 md:pt-48">
            ...
            {c.articlesNote && (
              <p className="mt-4 text-xs uppercase tracking-[0.18em] text-text-mute">
                {c.articlesNote}
              </p>
            )}
          </div>
                ...
                <li key={p.slug} className="bg-surface">
                  ...
                  >
                    <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.22em] text-text-mute">
                      <time dateTime={p.date}>{formatPostDate(p.date, dateLocale)}</time>
                      <span aria-hidden className="h-px w-6 bg-border" />
                      <span>{p.readingMinutes} {c.minRead}</span>
                    </div>
